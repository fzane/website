...
build-blog.mjs       regenerates the blog (blogposts/) from the Markdown corpus
blogposts-md/        canonical Markdown corpus (source) — one .md/post, w/ frontmatter
blogposts/           GENERATED by build-blog.mjs — do not hand-edit; index + one folder/post
about/  404.html
...
The blog is generated, not hand-written. `blogposts-md/` is the source of truth:
one Markdown file per post, named `YYYY-MM-DD-<slug>.md`, each opening with a
...
date: "2026-06-26"
subtitle: "An optional deck line"    # optional
description: "Short intro shown on the blog index card."
---

Body in Markdown…
```

`node build-blog.mjs` reads the corpus and writes, into `blogposts/`:

- `blogposts/index.html` — a card list of every post (newest first, text intros,
  no thumbnails), using the shared `.band`/`.rows` chrome like the project list.
- `blogposts/<slug>/index.html` — one self-contained reading page per post in the
  shared `.prose` style. Older/newer links are baked in at build time, so the blog
  needs no `manifest.json` entry and no `nav.js`.

The converter is a small hand-rolled Markdown parser inside `build-blog.mjs` — no
dependencies. To add or edit a post, change the Markdown in `blogposts-md/` and
re-run the build; never edit files under `blogposts/` directly. If you change
`site.css`, bump both its `?v=` links **and** the `CSS_V` constant in
`build-blog.mjs`.
